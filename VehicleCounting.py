...
def boxAndLineOverlap2(x_mid_point2, y_mid_point2, line_coordinates2):
	...
	if (x_mid_point2 >= x1_line2 and x_mid_point2 <= x2_line2+5) and		(y_mid_point2 >= y1_line2-10 and y_mid_point2 <= y2_line2+5):
		return True
	return False
...
def count_vehicles(idxs, boxes, classIDs, vehicle_count, previous_frame_detections, frame,vehicle_crossed_line_flag,line_coordinates,line_coordinates2):
	current_detections = {}
	# ensure at least one detection exists
	if len(idxs) > 0:
		# loop over the indices we are keeping
...
while True:
	...
	vehicle_count, current_detections = count_vehicles(idxs, boxes, classIDs, vehicle_count, previous_frame_detections, frame,vehicle_crossed_line_flag,line_coordinates,line_coordinates2)
